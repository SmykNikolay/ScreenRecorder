...
const videoRecorder = new Recorder(
  "video",
  (outputFilePath) =>
    `ffmpeg -f avfoundation -framerate 30 -video_size 1280x720 -i "0" -t 2 -c:v libx264 ${outputFilePath}`,
);
const screenRecorder = new Recorder(
  "screen",
  (outputFilePath) =>
    `ffmpeg -f avfoundation -pix_fmt uyvy422 -framerate 30 -video_size 1920x1080 -probesize 100M -i "3:none" -t 2 -c:v libx264 -preset fast -crf 23 -level 4.1 ${outputFilePath}`,
);
...
// schedule.scheduleJob("*/10 * * * *", () => merger.mergeVideos());
